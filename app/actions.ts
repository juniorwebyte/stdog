...
// Corrigir a função authenticateAdmin para garantir que funcione corretamente
export async function authenticateAdmin(username: string, password: string) {
  try {
    if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
      // Criar um token de sessão simples
      ...
      // Armazenar a sessão no servidor de forma simplificada
      const dataDir = path.join(process.cwd(), "data")
      if (!fs.existsSync(dataDir)) {
        fs.mkdirSync(dataDir, { recursive: true })
      }

      // Armazenar apenas o token atual para simplificar
      const sessionData = {
        token: sessionToken,
        username: username,
        createdAt: new Date().toISOString(),
...
// Simplificar a função checkAdminSession para melhorar a verificação
